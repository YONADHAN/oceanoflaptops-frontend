...
import {productService, categoryService} from "../../../apiServices/adminApiServices"
...
const ProductEdit = () => {
  ...
  const createImage = (url) =>
    ...
    const handleImageChange = (e, index) => {
      ...
      if (file && isValidImage(file)) {
        ...
        toast.error("Invalid file. Only JPEG, PNG, and WEBP under 5MB are allowed.");
      }
    };
  ...
  const handleCropSave = async () => {
    try {
      ...
      setImages((prevImages) => {
        ...
        updatedImages[cropperState.selectedIndex] = {
          file: croppedFile,
  ...
  const cropperModal = (
    <div className={`fixed inset-0 z-50 ${cropperState.showCropper ? 'block' : 'hidden'}`}>
          ...
          <div className="p-4 border-t space-y-4">
            <div className="flex items-center space-x-4">
              ...
              <input
                ...
                onChange={(e) => setCropperState(prev => ({ 
                  ...prev, 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
     ...
     const response = await productService.updateProduct(productId, productSubmissionData)
